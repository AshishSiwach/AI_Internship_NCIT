# -*- coding: utf-8 -*-
"""Gemma-2b_QLORA.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1PoAjyeaYWeFmXz8GzGxeau9kq6tRDk5M
...
class ModelConfig:
    ...
    PUSH_TO_HUB = True
    # Hugging Face repository ID (username/model-name)
    ...
    OUTPUT_DIR = "/content/drive/MyDrive/football_model_gemma/results"
    LOGGING_DIR = "/content/drive/MyDrive/football_model_gemma/logs"
...
"""### Format Prompt and Data Transformation for gemma-2b"""
...
def create_chat_prompt(example, config: ModelConfig):
    ...
    # Combining the instruction and the specific task into a single user prompt.
    user_prompt = f"{instruction}\n\n{claim}"

    model_answer = f"**Football Term:** {example[config.TARGET_FOOTBALL_COLUMN]}\n**Reasoning:** {example[config.TARGET_REASONING_COLUMN]}"

    # The message list for Gemma should containing 'user' and 'assistant' roles.
    messages = [
...
class BatchInferenceConfig:
    ...
    """
    # --- Model Selection ---
    # The Hugging Face Hub repository ID of fine-tuned LoRA adapters.
...
def load_model_and_tokenizer(adapter_path: str):
    ...
    try:
        # Get the base model name from the adapter's config using PeftConfig
...
def generate_prediction(model, tokenizer, claim_text: str, config: BatchInferenceConfig):
    ...
    # Defining the system and user prompts, exactly matching the training format.
    instruction = "You are a witty football commentator who refutes claims with a clever, direct analogy. You MUST first provide a short, punchy 'Football Term' you would hear on a broadcast, followed by a 'Reasoning' that explains the connection. Your response must strictly follow this two-part format."
    ...
    # The message list for Gemma only containing 'user' and 'assistant' roles.
    messages = [
        {"role": "user", "content": user_prompt}
    ]

    # Appling the template and get the input IDs.
...
class EvaluationConfig:
    ...
    OUTPUT_DATA_PATH = "/content/drive/MyDrive/Colab/gemma_evaluation_results_gemma-2b.csv"
    # The name of the file where the average evaluation metrics will be saved.
    MEAN_RESULTS_FILE_PATH = "/content/drive/MyDrive/Colab/gemma_mean_evaluation_results_gemma-2b.csv"
